...
class VAE(pl.LightningModule):
    ...
    def monotonic_reg(self, x_recon, zX, reg_lambda):
    ...
    def solve_ode(self, zX, init_u, t, total_time, time_points):
        ...
        def func(t, u):
            ...
            return self.ode_func(u, dx, dy, zX)
    ...
    def forward(self, x, x_t):
        ...
        init_u = x[:, 0, :, :]
        x_recon = self.solve_ode(zX, init_u, self.predefined_t, self.total_time, self.time_points)  # [B, T, H, W]
        #x_recon = x_recon + g_theta(x_recon)  # [B, T, H, W]

        return x, x_recon, zX_mean, zX_logvar, zX

    def loss_function(self, x, x_recon, zX_mean, zX_logvar, zX):
        ...
        if self.monotonic_regularisation:
            mono_reg = self.monotonic_reg(x_recon, zX, reg_lambda=self.reg_lambda)
    ...
    def training_step(self, batch, batch_idx):
        ...
        x, x_recon, zX_mean, zX_logvar, zX = self(x, x_t)

        loss = self.loss_function(x, x_recon, zX_mean, zX_logvar, zX)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.extrapolate:
            ...
            x_extrap = self.solve_ode(zX, init_u, full_t, self.extrap_time, self.extrap_points)
