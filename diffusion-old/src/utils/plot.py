...
def plot_hist(zX, param_path, path):
    learned_values = zX.detach().cpu().numpy().flatten()
    true_values = np.load(param_path)

    errors = learned_values - true_values[:, 0]

    df = pd.DataFrame(errors, columns=['zX'])
...
def test_plot_recon(x, x_t, recon_samples, dir):
    ...
    x = x.detach().cpu().numpy()[2]  # first sample
    x_t = x_t.detach().cpu().numpy()[2]
    idx = int(x.shape[2]/2)

    recons_stacked = np.stack(recon_samples, axis=0)
    mean_recon = np.mean(recons_stacked, axis=0)[2]
...
def test_plot_param(zX, dir, data_dir):
    plot_hist(zX, param_path=f"{data_dir}/test_param.npy", path=f"{dir}/test_param_hist.png")
    #plot_hist_adni(zX, path=f"{dir}/test_param_hist.png")

def test_plot_extrap(full_t, x_extrap, dir, data_dir):
    x_extrap = x_extrap.detach().cpu().numpy()[2]
...
def val_plot_recon(x, x_t, x_recon, zX, dir):
