...
def plot_recon(x, x_t, x_recon, path):
    ...
    for i in range(n_cols):
        # ground truth
        fig.add_trace(go.Heatmap(z=x[i], colorscale="Blues", showscale=(i == n_cols - 1), zmin=global_min, zmax=global_max, colorbar=colorbars[0] if i == n_cols - 1 else None), row=1, col=i + 1)
        # prediction
        fig.add_trace(go.Heatmap(z=x_recon[i], colorscale="Blues", showscale=(i == n_cols - 1), zmin=global_min, zmax=global_max, colorbar=colorbars[1] if i == n_cols - 1 else None), row=2, col=i + 1)
        # difference
...
def plot_hist(zX, zY, zR, param_path, path):
    true_values = np.hstack([zX.detach().cpu().numpy(), zY.detach().cpu().numpy(), zR.detach().cpu().numpy()])
    learned_values = np.load(param_path)

    errors = learned_values - true_values

    df = pd.DataFrame(errors, columns=['zX', 'zY', 'zR'])
    df_melted = df.melt(var_name='Parameter', value_name='Error')
    custom_colors = [px.colors.sequential.Plasma[0], px.colors.sequential.Plasma[1], px.colors.sequential.Plasma[2]]

    fig = px.histogram(df_melted, x="Error", color="Parameter", marginal="box", nbins=50, opacity=0.5, color_discrete_sequence=custom_colors, barmode="overlay")
...
def plot_extrap(full_t, x_extrap, param_path, path):
    ...
    true_extrap = get_sample(int(space), int(full_t[-1]), full_t, test_param[0][0], test_param[0][1], test_param[0][2])
    ...
    for i in range(n_cols):
        # true
        fig.add_trace(go.Heatmap(z=true_extrap[i], colorscale="Blues", showscale=(i == n_cols - 1), zmin=global_min, zmax=global_max, colorbar=colorbars[0] if i == n_cols - 1 else None), row=1, col=i + 1)
        # prediction
        fig.add_trace(go.Heatmap(z=x_extrap[i], colorscale="Blues", showscale=(i == n_cols - 1), zmin=global_min, zmax=global_max, colorbar=colorbars[1] if i == n_cols - 1 else None), row=2, col=i + 1)
        # difference
...
def test_plot_recon(x, x_t, recon_samples, dir):
    ...
    plot_uncertainty(x, recons_stacked, cross_section_idx= 12, path=f"{dir}/test_uncert.png")

def test_plot_param(zX, zY, zR, dir, data_dir):
    plot_hist(zX, zY, zR, param_path=f"{data_dir}/test_param.npy", path=f"{dir}/test_param_hist.png")
...
def val_plot_recon(x, x_t, x_recon, zX, zY, zR, dir):
    x = x.detach().cpu().numpy()[0]  # first sample
    x_t = x_t.detach().cpu().numpy()[0]
