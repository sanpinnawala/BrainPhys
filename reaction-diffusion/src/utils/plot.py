...
def plot_uncertainty(x, recons_stacked, cross_section_idx, path):
    ...
    fig.add_trace(go.Scatter(x=x_positions, y=true_x, mode='lines', line=dict(color='royalblue'), name='True'), row=1,
                  col=1)
    fig.add_trace(go.Scatter(x=x_positions, y=mean_x, mode='lines', line=dict(color='rgba(106,81,163,1)'),
                             name='Mean of Predictions'), row=1, col=1)
    fig.add_trace(
        go.Scatter(x=x_positions, y=max_x, mode='lines', line=dict(color='rgba(106,81,163,0)'), showlegend=False),
        row=1, col=1)
    fig.add_trace(go.Scatter(x=x_positions, y=min_x, mode='lines', fill='tonexty', fillcolor='rgba(106,81,163,0.2)',
                             line=dict(color='rgba(106,81,163,0)'), name='Range of Predictions'), row=1, col=1)
