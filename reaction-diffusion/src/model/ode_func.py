...
class ODEFunc(nn.Module):
    ...
    def forward(self, y, mask, dx, dy, zX, zR):
        ...
        # reaction term
        # true: du_dt += zR.unsqueeze(1) * y * (1-y)
        # m_a: du_dt += zR.unsqueeze(1) * y * (1-y) * (y-1)
        # m_b: du_dt += zR.unsqueeze(1) * y * y * (1-y)
        # m_c: du_dt += zR.unsqueeze(1) * y * (1-y) ** 2
