...
class VAE(pl.LightningModule):
    ...
    def monotonic_reg(self, x_recon, mask, zX, reg_lambda):
        ...
        for i in range(t):
            ...
            du_dt = func(u, mask, dx, dy, zX)  # [B, H, W]
    ...
    def solve_ode(self, zX, init_u, mask, t, total_time, time_points):
        ...
        def func(t, u):
            ...
            return self.ode_func(u, mask, dx, dy, zX)
    ...
    def forward(self, x, x_mask, x_t):
        zX_mean, zX_logvar = self.encoder(x, x_t)
        ...
        x_recon = self.solve_ode(zX, init_u, mask, self.predefined_t, self.total_time, self.time_points)  # [B, T, H, W]
        # x_recon = x_recon + g_theta(x_recon)  # [B, T, H, W]

        return x, x_recon, mask, zX_mean, zX_logvar, zX

    def loss_function(self, x, x_recon, mask, zX_mean, zX_logvar, zX):
        zX_prior_mean, zX_prior_logvar = self.log_normal_prior(self.alpha_range[0], self.alpha_range[1])

        # kl divergence
        kl_loss = self.kl_divergence(zX_mean, zX_logvar, zX_prior_mean, zX_prior_logvar)
        ...
        if self.monotonic_regularisation:
            mono_reg = self.monotonic_reg(x_recon, mask, zX, reg_lambda=self.reg_lambda)
    ...
    def training_step(self, batch, batch_idx):
        ...
        x, x_recon, mask, zX_mean, zX_logvar, zX = self(x, x_mask, x_t)

        loss = self.loss_function(x, x_recon, mask, zX_mean, zX_logvar, zX)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.trainer.current_epoch == (self.max_epochs - 1):
            val_plot_recon(x, x_t, x_recon, zX, self.artefacts)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.reconstruct:
            for i in range(self.infer_samples):
                _, x_recon, _, _, _, _ = self(x, x_mask, x_t)
                recon_samples.append(x_recon.detach().cpu().numpy())
            test_plot_recon(x, x_t, recon_samples, self.artefacts)

        # parameter analysis
        if self.param_analysis:
            _, _, _, _, _, zX = self(x, x_mask, x_t)
            test_plot_param(zX, self.artefacts, self.data_dir)

        # interpolation and extrapolation
        if self.extrapolate:
            _, _, _, _, _, zX = self(x, x_mask, x_t)
            full_t = np.sort(np.concatenate((x_t.detach().cpu().numpy()[0], self.extrap_t)))
            x_extrap = self.solve_ode(zX, init_u, mask, full_t, self.extrap_time, self.extrap_points)
